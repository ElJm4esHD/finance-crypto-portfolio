...
import { getCryptoPriceProvider, getMarketPriceProvider } from './prices/index.js';
...
app.log.info(`Datos en ${config.dataDir} · precios cripto: ${cryptoPrices.name} · precios CEDEARs/ETF: ${marketPrices.name}`);

setTimeout(dailyJob, 5_000);
const timer = setInterval(dailyJob, config.snapshotIntervalMinutes * 60_000);
